...
<%
	...
	rsArquivos.Open "select * from "&prefixoTabela&"arquivo_2 where ativo='s' and idFase="&id&" order by id desc", Conexao
%>
	...
	%>    		
			<tr class="gradeA">
				...
				<td><button type="button" class="btn btn-danger" onclick="location.href='_dados.asp?tipo=9&act=3&id=<%=rsArquivos("id")%>&idC=<%=id%>'">Excluir</button></td>
			</tr>		
	<%
	rsArquivos.MoveNext()
	wend
...
elseif act="3" then
	...
	editSQL = "UPDATE "&prefixoTabela&"arquivo_2 SET ativo='n', ultima_alteracao='"& ultima_alteracao &"' WHERE id="&id
	addRs.open editSQL, Conexao

	response.redirect("_dados.asp?tipo=9&act=1&id="&idC)
